  ...
  installPhase = ''
    mkdir $out &&
      ...
      makeWrapper \
        "$out/src/setup.sh" \
	"$out/bin/setup" \
	--set PATH "${pkgs.lib.makeBinPath [ pkgs.coreutils pkgs.docker docker-image-id docker-container-id uuid-parser pkgs.findutils docker-container-start-and-wait-for-healthy start-read-only-pass-container start-read-write-pass-container docker-image-load system-secrets-read-only-pass ] }" \
	--set STORE_DIR "$out" &&
