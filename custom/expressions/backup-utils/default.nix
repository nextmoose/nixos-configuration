...
stdenv.mkDerivation rec {
  ...
  installPhase = ''
    mkdir $out &&
      ...
      makeWrapper $out/scripts/bucket.sh $out/bin/bucket --set PATH ${lib.makeBinPath [ "/run/wrappers" coreutils utillinux lvm2 gnutar gzip cdrkit cdi2iso 	mdf2iso ccd2iso libisofs dvdisaster gnupg awscli ]} &&
