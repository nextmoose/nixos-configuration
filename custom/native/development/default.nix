...
stdenv.mkDerivation rec {
  ...
  installPhase = ''
    mkdir $out &&
      ...
      mkdir $out/bin &&
      makeWrapper $out/scripts/development.sh $out/bin/development --set PATH ${lib.makeBinPath [ coreutils pass gnupg git bash bash-completion openssh emacs "$out" utillinux maven which nodejs-slim-9 less ]} --set STORE_DIR "$out" &&
