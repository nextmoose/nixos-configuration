...
stdenv.mkDerivation rec {
  ...
  installPhase = ''
    mkdir $out &&
      ...
      chmod 0500 $out/scripts/init-development-environment.sh $out/scripts/post-commit.sh $out/scripts/pre-push.sh &&
      mkdir $out/bin &&
      makeWrapper $out/scripts/init-development-environment.sh $out/bin/init-development-environment --set PATH ${lib.makeBinPath [ init-read-only-pass pass git coreutils ]} --set STORE_DIR $out &&
