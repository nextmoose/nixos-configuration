...
stdenv.mkDerivation rec {
  ...
  installPhase = ''
    mkdir $out &&
      ...
      mkdir $out/bin &&
      makeWrapper $out/scripts/init-development-environment.sh $out/bin/init-development-environment --set PATH ${lib.makeBinPath [ init-read-only-pass pass git coreutils which ]} &&
