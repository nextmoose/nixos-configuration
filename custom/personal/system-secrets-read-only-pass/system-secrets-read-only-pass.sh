...
UUID=b5b09d3c-88b5-47ed-b15a-ab9bd25fe37b &&
	...
	--mount type=volume,source=$(docker container inspect --format '{{ range .Mounts }}{{ if eq .Destination "/home" }}{{ .Name }}{{ end }}{{ end }}' $(docker container ls --quiet --filter label=uuid=${UUID})),target=/home,readonly=false \
	pass \
	"${@}" &&
    true
